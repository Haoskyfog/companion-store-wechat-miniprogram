...
exports.main = async (event, context) => {
  ...
  try {
    ...
    // 7. 检查所有结果
    const results = await checkResults(orderId, reportId)
...
async function checkResults(orderId, reportId) {
  ...
  try {
    ...
    if (bossResult.data.length > 0) {
      const boss = bossResult.data[0]
      results.boss = {
        walletBalance: boss.walletBalance,
        totalConsumption: boss.totalConsumption,
    ...
    if (staffResult.data.length > 0) {
      ...
      results.staff = {
        approvedReports: staff.approvedReports
      }
      console.log('员工信息:')
